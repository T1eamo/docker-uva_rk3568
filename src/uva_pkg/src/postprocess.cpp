...
static const char *DEFAULT_LABEL_TXT_PATH = "model/apple.txt";
...
{
    int ret = 0;
    const char *resolved_label_path = label_path;

    if (resolved_label_path == NULL || resolved_label_path[0] == '\0')
    {
        resolved_label_path = DEFAULT_LABEL_TXT_PATH;
    }

    ret = loadLabelName(resolved_label_path, labels);
    if (ret <= 0)
    {
    ...
    if (ret != OBJ_CLASS_NUM)
    {
        printf("Warning: label count (%d) does not match OBJ_CLASS_NUM (%d)\n", ret, OBJ_CLASS_NUM);
    }
