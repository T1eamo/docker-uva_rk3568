...
static int should_try_rga(image_buffer_t *src_img, image_buffer_t *dst_img)
{
    const char *force_virtual_rga = getenv("FORCE_RGA_VIRTUAL");
    if (force_virtual_rga != NULL && strcmp(force_virtual_rga, "1") == 0) {
        return 1;
    }

    return (src_img->fd > 0) || (dst_img->fd > 0);
}
...
{
    int ret;
    static int warned_virtual_rga = 0;
    ...
    if (should_try_rga(src_img, dst_img)) {
        ret = convert_image_rga(src_img, dst_img, src_box, dst_box, color);
    } else {
        ret = -1;
        if (!warned_virtual_rga) {
            printf("skip RGA for virtual-memory buffers; using CPU preprocess. Set FORCE_RGA_VIRTUAL=1 to force the old path.\n");
            warned_virtual_rga = 1;
        }
    }
